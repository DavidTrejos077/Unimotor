...
public class PersonaREST {
	...
	public Response RegistrarPersona(Persona persona) {
		try {
			...
			return Response.status(200).entity("{\"mensaje\" :\" La persona se registró correctamente\"}")
					.type(MediaType.APPLICATION_JSON).build();
		} catch (Exception e) {
	...
	public Response ActualizarPersona(Persona persona) {
		try {
			...
			return Response.status(200).entity("{\"mensaje\" :\" La persona se actualizó correctamente\"}")
					.type(MediaType.APPLICATION_JSON).build();
		} catch (Exception e) {
	...
	public Response EliminarPersona(@PathParam("id") Integer id) {
		try {
			...
			return Response.status(200).entity("{\"mensaje\" :\" La persona se eliminó correctamente\"}")
					.type(MediaType.APPLICATION_JSON).build();
		} catch (Exception e) {
